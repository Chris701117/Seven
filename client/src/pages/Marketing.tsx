...
import { Grid2X2, ListIcon, Plus, BarChart3, LayoutGrid, RefreshCcw } from 'lucide-react';
...
export default function Marketing() {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) => {
      return apiRequest(`/api/marketing-tasks/${id}`, {
        method: 'DELETE'
      });
    },
    onSuccess: () => {
  ...
  // 根據類別分組任務
  const tasksByCategory = filteredTasks.reduce<Record<string, MarketingTask[]>>((acc, task) => {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardDescription>不同類別的行銷任務數量分布</CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardDescription>各狀態行銷任務數量分布</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="flex flex-col space-y-2">
