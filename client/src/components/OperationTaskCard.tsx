...
interface OperationTaskCardProps {
  task: OperationTask;
  onDelete: () => void;
  layout: "grid" | "list";
}
...
export default function OperationTaskCard({ task, onDelete, layout }: OperationTaskCardProps) {
        ...
        <Card className={`w-full h-full flex flex-col ${isOverdue() ? 'border-red-300' : ''}`}>
          ...
          <CardContent className="py-2 flex-grow">
            <div className="text-sm text-gray-600 max-h-20 overflow-hidden">
              {task.content || <span className="text-gray-400 italic">無任務描述</span>}
            </div>
          </CardContent>
          ...
          <CardFooter className="flex flex-col items-start pt-2 border-t">
            ...
            <div className="flex justify-between w-full mt-2">
              ...
              <Button size="sm" variant="outline" className="text-red-600 hover:text-red-700 hover:bg-red-50" onClick={onDelete}>
